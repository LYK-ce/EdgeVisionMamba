...
from mamba_ssm.ops.selective_scan_interface import (
    selective_scan_fn, selective_scan_ref, selective_fused_scan_fn, selective_scan_simd_fn,
    selective_scan_simd_fixlen_fn, selective_fused_scan_simd_fn, selective_fused_scan_simd_fixlen_fn
)
...
class Mamba(nn.Module):
    def __init__(
        ...
        use_simd_scan=False,  # 使用SIMD优化的selective scan
        use_simd_fixlen_scan=False,  # 使用SIMD+Fixlen优化的selective scan
        use_simd_fused_scan=False,  # 使用SIMD优化的fused双向selective scan
        use_simd_fused_fixlen_scan=False,  # 使用SIMD+Fixlen优化的fused双向selective scan
    ):
        ...
        self.use_simd_scan = use_simd_scan
        self.use_simd_fixlen_scan = use_simd_fixlen_scan
        self.use_simd_fused_scan = use_simd_fused_scan
        self.use_simd_fused_fixlen_scan = use_simd_fused_fixlen_scan
    ...
    def _forward_reference(self, xz, A, seqlen, batch, dim, conv_state, ssm_state, inference_params,
        ...
        if self.use_simd_fixlen_scan:
            # 使用SIMD+Fixlen联合优化的selective scan
            y = selective_scan_simd_fixlen_fn(
                x,
                dt,
                A,
                B,
        ...
        elif self.use_simd_scan:
            # 使用SIMD优化的selective scan
            y = selective_scan_simd_fn(
                x,
                dt,
                A,
                B,
    ...
    def _forward_fuse_reference(self, xz, A_fwd, A_bwd, seqlen, batch, dim, conv_state, ssm_state, inference_params):
        ...
        # 根据SIMD优化选项选择实现
        if self.use_simd_fused_fixlen_scan:
            # 使用SIMD+Fixlen联合优化的fused双向扫描（最高性能）
            out = selective_fused_scan_simd_fixlen_fn(
                dt_fwd, dt_bwd,
                A_fwd, A_bwd,
                B_fwd, B_bwd,
        ...
        elif self.use_simd_fused_scan:
            # 使用SIMD优化的fused双向扫描
            out = selective_fused_scan_simd_fn(
                dt_fwd, dt_bwd,
                A_fwd, A_bwd,
                B_fwd, B_bwd,
                x_fwd_conv, x_bwd_conv_flip,
