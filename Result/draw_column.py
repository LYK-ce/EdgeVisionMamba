...
Outputs vector PDF format for high-quality academic publication.

Usage:
    python draw_column.py --model vim_5m --configs "Python Original,SIMD" --output vim_5m_compare.pdf
    python draw_column.py --model vim_10m --all --output vim_10m_all.pdf
    python draw_column.py --model vim_5m --speedup --baseline "Python Original" -o speedup.pdf
...
# Academic style configuration (following draw.md specifications)
# Font sizes optimized for dual-column paper layout
ACADEMIC_STYLE = {
    'font.family': 'serif',
...
# Color palette for different optimization configs (NATURE palette from draw.md)
COLOR_PALETTE = {
    'Python Original': '#CC247C',        # Magenta
    'LO': '#E95351',                     # Red (Loop Optimization)
    'BSF': '#F7A24F',                    # Orange (Bidirectional Fusion)
    'LO + BSF': '#FBEB66',               # Yellow
    'SV': '#4EA660',                     # Green (SIMD)
    'SV + LO': '#79CAFB',                # Light Blue
    'SV + BSF': '#5C9AD4',               # Blue
    'SV + LO + BSF': '#AA77E9',          # Purple
}
...
# Mapping from original names to abbreviated names for legend display
CONFIG_NAME_MAP = {
    'Python Original': 'Python Original',
    'Loop Optimization': 'LO',
    'Bidirectional Fusion': 'BSF',
    'Loop + Fusion': 'LO + BSF',
    'SIMD': 'SV',
    'SIMD + Loop': 'SV + LO',
    'SIMD + Fusion': 'SV + BSF',
    'SIMD + Loop + Fusion': 'SV + LO + BSF'
}
...
DEVICE_NAMES = {
    'Laptop 1': 'Platform 1',
    'Laptop 2': 'Platform 2',
    'ARM64_Raspberry_Pi_5': 'Platform 3',
    'RK3588 8G': 'Platform 4'
}
...
def Parse_Xlsx(file_path: str) -> dict:
    ...
    for idx, row in df.iterrows():
        ...
        if 'Laptop' in second_cell or 'ARM' in second_cell or 'Intel' in second_cell:
        ...
        if current_model and devices and first_cell in known_configs:
            config_name = CONFIG_NAME_MAP.get(first_cell, first_cell)
...
def Parse_Csv(file_path: str) -> dict:
    ...
    for line in lines:
        ...
        if current_model and ',' in line and not line.startswith('配置'):
            parts = line.split(',')
            config_name = CONFIG_NAME_MAP.get(parts[0], parts[0])
...
def Main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  python draw_column.py --model vim_5m --configs "Python Original,SIMD" -o compare.pdf
  python draw_column.py --model vim_10m --all -o all_configs.pdf
  python draw_column.py --model vim_5m --speedup --baseline "Python Original" -o speedup.pdf
  python draw_column.py --model vim_5m --multi-device --all -o multi.pdf
  python draw_column.py --list-models
  python draw_column.py --list-configs
        """
    )
    ...
    parser.add_argument('--output', '-o', type=str, default='output.pdf', help='Output file path (pdf/png/svg)')
