...
"""
热力图绘制脚本
读取compare.xlsx的WHCompare sheet，展示模型宽度×深度对准确率的影响
"""
...
def Load_Data(filepath, sheet_name='WHCompare'):
    """加载compare.xlsx的WHCompare sheet"""
    df = pd.read_excel(filepath, sheet_name=sheet_name, index_col=0)
    return df

def Draw_Heatmap(df, output_path, title='Model Width vs Depth - Accuracy', 
    ...
    # 设置标签
    ax.set_xlabel('Model Width', fontweight='bold')
    ax.set_ylabel('Model Depth', fontweight='bold')
...
def main():
    parser = argparse.ArgumentParser(description='绘制模型配置热力图')
    parser.add_argument('-i', '--input', type=str, 
                        default='compare.xlsx',
                        help='输入文件路径 (默认: compare.xlsx)')
    parser.add_argument('-s', '--sheet', type=str,
                        default='WHCompare',
                        help='Sheet名称 (默认: WHCompare)')
    parser.add_argument('-o', '--output', type=str,
                        default='Img/wh_compare_heatmap.png',
                        help='输出文件路径 (默认: Img/wh_compare_heatmap.png)')
    parser.add_argument('-t', '--title', type=str,
                        default='Model Width vs Depth - Accuracy',
                        help='图表标题')
    ...
    print(f"行索引(深度): {df.index.tolist()}")
    print(f"列名(宽度): {df.columns.tolist()}")
